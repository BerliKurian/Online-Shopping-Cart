...
router.get('/user-home', async (req, res) => {
  try {
    ...
    products = products.map(product => {
      ...
      const secondImagePath = path.join(__dirname, '../public/newimages', `${productId}_1.jpg`);
      product.hasSecondImage = fs.existsSync(secondImagePath);
      return product;
    });
...
router.get('/orderhistory', isAuthenticated, async (req, res) => {
  try {
    ...
    let orders = await userHelper.getProductHistory();
    //let product=await userHelper.getProductHistory(req.session.user.id)
